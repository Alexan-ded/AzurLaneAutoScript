...
class Meta(UI, MapEventHandler):
    ...
    def digit_ocr_point_and_check(self, button: Button, check_number: int):
        point_ocr = MetaDigitCounter(
            button, letter=(235, 235, 235), threshold=160, name="POINT_OCR"
        )
...
class OpsiAshBeacon(Meta):
    ...
    def _satisfy_attack_condition(self):
        ...
        if self.appear(BEACON_LIST, offset=(20, 20)):
            ...
            if _server_support() and self.config.OpsiAshBeacon_OneHitMode:
                ...
                if damage > 0:
                    logger.info(
                        "Enable OneHitMode and meta damage is "
                        + str(damage)
        ...
        while 1:
            ...
            if self.appear(
                ...
                self.appear_then_click(
                    META_INNER_PAGE_NOT_DAMAGE, offset=(20, 20), interval=2
                )
                continue
    ...
    def _pre_attack(self):
        ...
        if self.appear(BEACON_LIST, offset=(20, 20)):
            ...
            if (
                self.config.OpsiAshBeacon_OneHitMode
                or self.config.OpsiAshBeacon_RequestAssist
            ):
    ...
    def _ask_for_help(self):
        ...
        while 1:
            ...
            if self.appear(BEACON_REWARD, offset=(30, 30)):
                logger.info(
                    "META finished just after calling assist, ignore meta assist"
                )
                return False
            # Click
    ...
    def _dossier_auto_attack(self):
        ...
        while 1:
            ...
            if timeout.reached():
                logger.warning(
                    "Run _dossier_auto_attack timeout, probably because META_AUTO_ATTACK_START was missing"
                )
                return False
            # Finished by others
            if self.appear(BEACON_REWARD, offset=(30, 30)):
            ...
            if self.appear_then_click(
                META_AUTO_ATTACK_CONFIRM, offset=(5, 5), interval=3
            ):
                continue
            if self.appear_then_click(
                META_AUTO_ATTACK_START, offset=(5, 5), interval=3
            ):
                continue
            # Wrongly entered BATTLE_PREPARATION
            if self.appear(BATTLE_PREPARATION, offset=(30, 30), interval=2):
    ...
    def _begin_meta(self):
        ...
        if self.appear(ASH_SHOWDOWN, offset=(30, 30), interval=2):
            ...
            if (
                ...
                if self.appear_then_click(
                    META_MAIN_DOSSIER_ENTRANCE, offset=(20, 20), interval=2
                ):
        ...
        # Page beacon
            ...
            return True
        # Page dossier
        elif _server_support() and self.appear(
            DOSSIER_LIST, offset=(20, 20), interval=2
        ):
            if (
                self.config.OpsiAshBeacon_AttackMode == "current_dossier"
                and self._check_dossier_point()
            ):
                if self.appear_then_click(
                    META_BEGIN_ENTRANCE, offset=(20, 20), interval=2
                ):
    ...
    def _get_state(self):
        ...
        # Page beacon or dossier
        elif self.appear(BEACON_LIST, offset=(20, 20)) or self.appear(
            DOSSIER_LIST, offset=(20, 20)
        ):
            if self.appear(HELP_ENTER, offset=(30, 30)):
    ...
    def _begin_beacon(self):
        ...
        if not _server_support():
            logger.info(
                "Server not support dossier beacon and OneHitMode, please contact the developer."
            )
        self._ensure_meta_page()
        self._attack_meta()
...
class AshBeaconAssist(Meta):
    ...
    def _make_an_attack(self):
        ...
        def expected_end():
            ...
            if self.appear_then_click(
                BEACON_LIST, offset=(-20, -5, 300, 5), interval=2
            ):
                return False
            if self.appear(ASH_SHOWDOWN, offset=(30, 30), interval=2):
    ...
    def _ensure_meta_level(self):
        ...
        if current < tier:
            logger.info(
                f"Tier {tier} beacon not found after 5 trial, use current beacon"
            )
